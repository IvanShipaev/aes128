//
//
//

#include "aes_128.h"
#include <string.h>

#define BLOCKSIZE 16

#ifndef AES_CFG_EXTERNAL

#define xtime(a) (((a)<0x80)?((a)<<1):(((a)<<1)^0x1b))

static const unsigned char STable[] =
{
	
};

static const unsigned char SiTable[] =
{
	
};

static const unsigned char x1[] =
{
	
};

static const unsigned char x2[] =
{
	
};

static const unsigned char x3[] =
{
	
};

/*********************************************************************
* Function: void DecodeShiftRow(unsigned char* stateTable)
*
* PreCondition:
*
* Input: stateTable - array of data to be shifted
*
* Output:
*
* Side Effects: values of the inputed data changed
*
* Overview: None
*
* Note: None
********************************************************************/
static void DecodeShiftRow (unsigned char * stateTable)
{
	unsigned char temp;

	/* first row (row 0) unchanged */

	/* second row (row 1) shifted left by three (or right by one) */
	temp=stateTable[1];
	stateTable[1]=stateTable[13];
	stateTable[13]=stateTable[9];
	stateTable[9]=stateTable[5];
	stateTable[5]=temp;

	/* third row (row 2) shifted left by two */
	temp=stateTable[2];
	stateTable[2]=stateTable[10];
	stateTable[10]=temp;
	temp=stateTable[14];
	stateTable[14]=stateTable[6];
	stateTable[6]=temp;

	/* fourth row (row 3) shifted left by one */
	temp=stateTable[7];
	stateTable[7]=stateTable[11];
	stateTable[11]=stateTable[15];
	stateTable[15]=stateTable[3];
	stateTable[3]=temp;
}

/*********************************************************************
* Function: void DecKeySchedule(unsigned char* key)
*
* PreCondition:
*
* Input: key - array of the key data
*
* Output:
*
* Side Effects: values of the key changed to next key in decrypt
*
* Overview: None
*
* Note: None
********************************************************************/
static void DecKeySchedule(unsigned char * key,unsigned char * _rcon)
{
	/* column 4 */
	key[12]^=key[8];
	key[13]^=key[9];
	key[14]^=key[10];
	key[15]^=key[11];

	/* column 3 */
	key[8]^=key[4];
	key[9]^=key[5];
	key[10]^=key[6];
	key[11]^=key[7];

	/* column 2 */
	key[4]^=key[0];
	key[5]^=key[1];
	key[6]^=key[2];
	key[7]^=key[3];

	/* column 1 */
	key[0]^=STable[key[13]];
	key[1]^=STable[key[14]];
	key[2]^=STable[key[15]];
	key[3]^=STable[key[12]];

	key[0]^=*_rcon;
	if ((*_rcon&0x01)!=0x00)
	{
		*_rcon = 0x80;
	}
	else
	{
		*_rcon >>= 1;
	}

}

/*********************************************************************
* Function: void EncKeySchedule(unsigned char* key)
*
* PreCondition:
*
* Input: key - array of the key data
*
* Output: 
*
* Side Effects: values of the key changed
*
* Overview: None
*
* Note: None
********************************************************************/
static void EncKeySchedule (unsigned char * key,unsigned char * _rcon)
{
	/* column 1 */
	key[0]^=STable[key[13]];
	key[1]^=STable[key[14]];
	key[2]^=STable[key[15]];
	key[3]^=STable[key[12]];

	key[0]^=*_rcon;
	*_rcon = xtime(*_rcon);

	/* column 2 */
	key[4]^=key[0];
	key[5]^=key[1];
	key[6]^=key[2];
	key[7]^=key[3];

	/* column 3 */
	key[8]^=key[4];
	key[9]^=key[5];
	key[10]^=key[6];
	key[11]^=key[7];

	/* column 4 */
	key[12]^=key[8];
	key[13]^=key[9];
	key[14]^=key[10];
	key[15]^=key[11];
}

/*********************************************************************
* Function: void EncodeShiftRow(unsigned char* stateTable)
*
* PreCondition:
*
* Input: stateTable - the data needing to be shifted
*
* Output: 
*
* Side Effects: values of the inputed data changed
*
* Overview: None
*
* Note: None
********************************************************************/
static void EncodeShiftRow(unsigned char * stateTable)
{
	unsigned char temp;

	/* first row (row 0) unchanged */

	/* second row (row 1) shifted left by one */
	temp=stateTable[1];
	stateTable[1]=stateTable[5];
	stateTable[5]=stateTable[9];
	stateTable[9]=stateTable[13];
	stateTable[13]=temp;
	
	/* third row (row 2) shifted left by two */
	temp=stateTable[2];
	stateTable[2]=stateTable[10];
	stateTable[10]=temp;
	temp=stateTable[14];
	stateTable[14]=stateTable[6];
	stateTable[6]=temp;

	/* fourth row (row 3) shifted left by three (or right by one) */
	temp=stateTable[3];
	stateTable[3]=stateTable[15];
	stateTable[15]=stateTable[11];
	stateTable[11]=stateTable[7];
	stateTable[7]=temp;
}


/*********************************************************************
 * Function:        void AESEncode(unsigned char* block, unsigned char* key)
 *
 * PreCondition:    None
 *
 * Input:           block - 16 byte block of data to be encrypted
 *						    (padded if needed)
 *
 * Output:          None
 *
 * Side Effects:    block values changed
 *
 * Stack Requirements: None
 *
 * Overview:        None
 *
 * Note:            None
 ********************************************************************/
static void AESEncode(unsigned char * block, unsigned char * key)
{
	unsigned char _roundCounter;
	unsigned char _rcon;
	unsigned char i;

	/* initiate round counter = 10 */
	_roundCounter=0x0A;

	_rcon=0x01;

	/* key addition */
	for(i=0;i<BLOCKSIZE;i++)
	{
		block[i]^=key[i];
	}

	do
	{

		/* s table substitution */
		for(i=0;i<BLOCKSIZE;i++)
		{
			block[i]=STable[block[i]];
		}

		/*   encode shift row */
		EncodeShiftRow(block);

		/*   if round counter != 1 */
		if (_roundCounter!=0x01)
		{
			unsigned char aux0,aux1,aux2,aux3;

			/*     mix column */
			for(i=0;i<16;i+=4)
			{
				aux1=block[i+0]^block[i+1];
				aux3=block[i+2]^block[i+3];
				aux0=aux1^aux3;
				aux2=block[i+2]^block[i+1];

				aux1=xtime(aux1);
				aux2=xtime(aux2);
				aux3=xtime(aux3);

				block[i+0]=aux0^aux1^block[i+0];
				block[i+1]=aux0^aux2^block[i+1];
				block[i+2]=aux0^aux3^block[i+2];
				block[i+3]=block[i+0]^block[i+1]^block[i+2]^aux0;
			}
		}

		/*   encode key schedule */
		EncKeySchedule(key,&_rcon);

		/*   key addition */
		for(i=0;i<BLOCKSIZE;i++)
		{
			block[i]^=key[i];
		}

		_roundCounter--;
	}
	while (_roundCounter!=0x00);
}

/*********************************************************************
 * Function:        void AESDecode(unsigned char* block, unsigned char* key)
 *
 * PreCondition:    None
 *
 * Input:           block - 16 byte block of data to be deciphered 
 *
 * Output:          None
 *
 * Side Effects:    block values changed
 *
 * Stack Requirements: None
 *
 * Overview:        None
 *
 * Note:            None
 ********************************************************************/
static void AESDecode(unsigned char * block, unsigned char * key)
{
	unsigned char i;
	unsigned char _roundCounter;
	unsigned char _rcon;

	/* reconstruct key */
	i=1;

	_rcon = 0x36;

	/* initiate round counter = 10 */
	_roundCounter = 10;

	/* key addition */
	for(i=0;i<BLOCKSIZE;i++)
	{
		block[i] ^= key[i];
	}

	do
	{

		if( _roundCounter != 10)
		{
			/* inverse mix column */
			/*     mix column */

			{
				unsigned char temp0,temp1,temp2,temp3;
				for(i=0;i<16;i+=4)
				{
					temp3=x3[block[i+0x00]]^x3[block[i+0x01]]^x3[block[i+0x02]]^x3[block[i+0x03]]^block[i+0x00]^block[i+0x01]^block[i+0x02]^block[i+0x03];
					temp0=x2[block[i+0x00]]^x1[block[i+0x00]]
							  ^x1[block[i+0x01]]
							  ^x2[block[i+0x02]]
							  ^temp3^block[i+0x00];
					temp1=x2[block[i+0x01]]^x1[block[i+0x01]]
							  ^x1[block[i+0x02]]
							  ^x2[block[i+0x03]]
							  ^temp3^block[i+0x01];
					temp2=x2[block[i+0x02]]^x1[block[i+0x02]]
							  ^x1[block[i+0x03]]
							  ^x2[block[i+0x00]]
							  ^temp3^block[i+0x02];
					temp3^=x2[block[i+0x03]]^x1[block[i+0x03]]
							  ^x1[block[i+0x00]]
							  ^x2[block[i+0x01]]
							  ^block[i+0x03];
					block[i+0]=temp0;
					block[i+1]=temp1;
					block[i+2]=temp2;
					block[i+3]=temp3;
				}
			}
			
		}

		/* s table substitution */
		for(i=0;i<BLOCKSIZE;i++)
		{
			block[i]=SiTable[block[i]];
		}

		/* decode shift row */
		DecodeShiftRow(block);

		/* decode key schedule */ 
		DecKeySchedule(key,&_rcon);

		for(i=0;i<BLOCKSIZE;i++)
		{
			block[i] ^= key[i];
		}

		_roundCounter--;
	}
	while( _roundCounter !=0);
}

//---------------------------------------------------
void aes_encode_ecb(uint8_t block[16], const uint8_t key[16])
{
	uint8_t tkey[16];
	memcpy(tkey, key, 16);
	AESEncode(block, tkey);
}

void aes_decoder_ecb(uint8_t block[16], const uint8_t key[16])
{
	uint8_t tkey[16];
	memcpy(tkey, key, 16);
	AESDecode(block, tkey);
}

#endif /* AES_CFG_EXTERNAL */

void aes_ctr_init(aes_ctr_t *ctr, const uint8_t key[16], const uint8_t iv[16], uint32_t offset)
{
	uint16_t tmp = 0;
	uint32_t block = offset/BLOCKSIZE;

	ctr->offset = offset & (BLOCKSIZE-1);
	memcpy(ctr->key, key, BLOCKSIZE);
	memcpy(ctr->coun, iv, BLOCKSIZE);

	for(int i = BLOCKSIZE-1; i >= 0; i--) {
		tmp = ctr->coun[i] + (block&0xff) + (tmp&0xff);
		ctr->coun[i] = tmp&0xff;
		block >>= 8;
		tmp >>= 8;
	}
	memcpy(ctr->data, ctr->coun, BLOCKSIZE);
	aes_encode_ecb(ctr->data, ctr->key);
    for(int i = BLOCKSIZE-1; i >= 0; i--)
    	if(++ctr->coun[i]) break;
}

void aes_ctr_update(aes_ctr_t *ctr, void* buf, uint32_t len)
{
	uint8_t *tbuf = buf;

	while(len) {
		while(ctr->offset < BLOCKSIZE) {
			*tbuf ^= ctr->data[ctr->offset++];
			tbuf++;
			if (--len==0) return;
		}
		ctr->offset = 0;
		memcpy(ctr->data, ctr->coun, BLOCKSIZE);
		aes_encode_ecb(ctr->data, ctr->key);
	    for(int i = BLOCKSIZE-1; i >= 0; i--)
	    	if(++ctr->coun[i]) break;
	}
}
